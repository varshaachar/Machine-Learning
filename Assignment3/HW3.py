...
X_ica = X_r
print('Kurtosis:', stats.kurtosis(X_ica))
...
for n_clusters in range_n_clusters:
    ...
    plt.suptitle(("KMeans clustering using Randomized Projection feature transformation "
                  "with n_clusters = %d" % n_clusters),
                 fontsize=14, fontweight='bold')
...
for n_clusters in range_n_clusters:
    ...
    plt.suptitle(("KMeans clustering using K Best "
                  "with n_clusters = %d" % n_clusters),
                 fontsize=14, fontweight='bold')
...
for n_clusters in range_n_clusters:
    ...
    ax.set_xlabel("Feature space for the 1st feature")
    ax.set_ylabel("Feature space for the 2nd feature")
    plt.suptitle(("Clusters plot for EM clustering on Random Projection data "
                  "with n_clusters = %d" % n_clusters), fontsize=14, fontweight='bold')
...
clf = MLPClassifier(hidden_layer_sizes=(100, 10, 5, 15), random_state=0, solver="adam", activation="tanh", alpha=1e-1)
...
#MLPClassifier(hidden_layer_sizes=(20, 5), random_state=0, solver="lbfgs")
clf = MLPClassifier(hidden_layer_sizes=(100, 10, 5, 15), random_state=0, solver="adam", activation="tanh", alpha=1e-1)
...
print('this  what is taking time')
cluster_labels = clusterer.predict(X)
print("NMI score: %.6f" % normalized_mutual_info_score(y, cluster_labels))
...
plt.suptitle(("K Means clustering on raw sample data "
              "with n_clusters = %d" % n_clusters),
             fontsize=14, fontweight='bold')
...
for n_clusters in range_n_clusters:
    ...
    colors = cm.spectral(cluster_labels.astype(float) / n_clusters)
    ax.scatter( X_test.iloc[:, 0], X_test.iloc[:, 1], marker='.', s=200, lw=0, alpha=0.8,
                c=colors, edgecolor='k')
...
for n_clusters in range_n_clusters:
    ...
    colors = cm.spectral(cluster_labels.astype(float) / n_clusters)
    plt.scatter( X_test.iloc[:, 0], X_test.iloc[:, 1], marker='.', s=200, lw=0, alpha=0.8,
                c=colors, edgecolor='k')
